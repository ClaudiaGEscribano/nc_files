...
SISymk <- stack("SISymk.nc", varname="rsds")
idx <- seq(as.Date('1989-07-02'), as.Date('2008-07-02'), 'year')
SISym <- setZ(SISymk, idx)
names(SISym) <- idx

## mask fot the IP

ext <- as.vector(extent(SISymk))
...
stdAnual <- stack("std_anual.nc", varname="rsds")
variabilidadAnual <- stdAnual/mean(IP)
levelplot(variabilidadAnual, contour=TRUE)
...
## cdo timselstd,20[,[365]] SISdkw.nc std_diaria.nc
